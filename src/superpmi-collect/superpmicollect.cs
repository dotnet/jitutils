...
namespace SuperPMICollection
{
    internal class Global
    {
        ...
        {
            CoreRoot = System.Environment.GetEnvironmentVariable("CORE_ROOT");
    ...
    internal class SuperPMICollectionClass
    {
        ...
        private static int RunProgram(string program, string arguments)
        {
            ...
            else if (program.LastIndexOf(".dll") != -1)
            {
               string programArgumentSep = String.IsNullOrEmpty(arguments) ? "" : " ";
               arguments = program;
               program = "dotnet";
    ...
    internal class Program
    {
        private static void Usage()
        {
            ...
            Console.WriteLine(" "); // Space needed to force newline
            ...
            Console.WriteLine(" "); // Space needed to force newline
            Console.WriteLine("On success, the return code is 0.");
        }
            ...
            if (args.Length > 0)
            {
                for (int i = 0; i < args.Length; i++)
                {
                    switch (args[i])
                    {
                        ...
                        case "-temp":
                            i++;
                            Console.WriteLine("Setting temp path to " + args[i]);
                            if (i >= args.Length)
                            {
